...
function openSession(socket: FakeSocket) {
  socket.open();
...
it("opens a session and sends only the WebSocket spin command and matches the response ID", async () => {
...
it("handles server errors and malformed results", async () => {
  ...
  socket.message({
    type: "slot.result",
    requestId: JSON.parse(socket.sent[1]).requestId,
    result: { ...result, matrix: [[99]] },
  });
...
it("validates winning cell coordinates against the matrix", () => {
  ...
  expect(
    isSpinResult({
      ...result,
      wins: [{ id: 0, positions: [null] }],
      winCount: 1,
    }),
  ).toBe(false);
});

it("reuses an unresolved spin ID after timeout so retries cannot double-charge", async () => {
  client = new GameSocket("ws://localhost/ws/games", vi.fn());
  const socket = FakeSocket.instances[0];
  openSession(socket);
  const failed = expect(client.spin()).rejects.toThrow("timed out");
  const first = JSON.parse(socket.sent[0]);
  vi.advanceTimersByTime(8000);
  await failed;
  const retry = client.spin();
